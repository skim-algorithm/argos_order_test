...
func (c *Client) parseRequest(r *request, opts ...RequestOption) (err error) {
	...
	if r.recvWindow > 0 {
		...
		// timestamp 이슈로 default 5000ms -> 60000ms로 늘림
		r.recvWindow = 60000
		r.setParam(recvWindowKey, r.recvWindow)
	}
	if r.secType == secTypeSigned {
		// 미래 시각에 대해서는 recvWindow와 무관하게 1000ms까지만 허용하므로
		// offset보다 5000 ms만큼 더 이전으로 돌려 Binance timestamp가 변동하더라도 서버 시각을 넘지 않도록 한다.
		r.setParam(timestampKey, currentTimestamp()-(c.TimeOffset+5000))
	}
